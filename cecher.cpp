/*
 Cecher: efficient computation of Cech persistence barcodes
...
template <typename T> struct sphere_t : std::pair<T,index_dim_t>{
    sphere_t(T _diameter, index_dim_t _support) : std::pair<T,index_dim_t> { _diameter,_support} {}
};
...
struct diameter_entry_t : std::tuple<PT, entry_t, index_dim_t> {
    ...
    //sphere_t conversion to diameter_entry_t
    diameter_entry_t(sphere_t<PT> _ball, index_t _index, coefficient_t _coefficient)
            : diameter_entry_t(_ball.first, make_entry(_index, _coefficient),_ball.second) {}
#ifndef USE_RATIONALS
    diameter_entry_t(sphere_t<ET> _ball, index_t _index, coefficient_t _coefficient)
...
template <typename T> class m_sphere {
...
public:
    ...
    m_sphere(const std::vector<std::vector<T>>& _points,
    ...
    sphere_t<T> construct(const std::vector<index_t>& _vertices, const index_t j=-1) {
        ...
        while (true){
            ...
            if(dim==m-1 || (dim==1 && is_valid()) || (dim>1 && CM_inv.size()>0 && is_optimal() &&
                ...
                return sphere_t<T>((dim>1)? radius*4: diameter,index_dim_t {support, dim});
            }
    ...
    sphere_t<T> construct_cofacet(const std::vector<index_t>& _vertices, const index_t j=-1) {
...
template<> PT m_sphere<PT>::get_dist(const index_t& a, const index_t& b){
    return dist(a,b);
}
#ifndef USE_RATIONALS
template<> ET m_sphere<ET>::get_dist(const index_t& a, const index_t& b){
...
#ifndef USE_RATIONALS
template<> void m_sphere<CGAL::Interval_nt<>>::throw_to_exact_type(){
    throw CGAL::Uncertain_conversion_exception("");
}
#endif
template<> void m_sphere<CGAL::Exact_rational>::throw_to_exact_type(){}

//lean miniball-algorithm for 2-simplices
template <typename T> class m_2_sphere {
...
public:
    ...
    m_2_sphere(const table &_binomial_coeff, const distance_matrix &_dist) :
    ...
    sphere_t<PT> construct(const std::vector<index_t>& _vertices, const index_t& j,const index_t& cofacet_index){
        //reset
        vertices=_vertices;
        ...
        return sphere_t<PT>(cofacet_diameter,index_dim_t {support, (max_edge_index==-1)? 2 : 1});
    }
};
    ...
    cecher(DistanceMatrix&& _dist, index_t _dim_max, PT _threshold, PT _ratio,
           coefficient_t _modulus, mtx_PT&& _points_PT, mtx_ET&& _points_ET)
            : dist(std::move(_dist)), n(dist.size()),
              ...
              m_sphere_ET(m_sphere<ET>(points_ET,binomial_coeff,dist)) {}
...
template <> class cecher<distance_matrix>::simplex_coboundary_enumerator {
    ...
    m_sphere<PT> m_sphere_PT;
    m_sphere<ET> m_sphere_ET;
    m_2_sphere<PT> m_2_sphere_PT;
...
public:
    ...
    simplex_coboundary_enumerator(const cecher& _parent) : modulus(_parent.modulus), dist(_parent.dist),
                                                           ...
                                                           m_sphere_PT(m_sphere<PT>(points_PT,binomial_coeff,dist)),
                                                           m_sphere_ET(m_sphere<ET>(points_ET,binomial_coeff,dist)),
                                                           m_2_sphere_PT(m_2_sphere<PT>(binomial_coeff,dist)){}
...
#ifndef USE_RATIONALS
                upper_tri_mtx<ET> CM_inv_a= m_sphere_ET.circumsphere(supp_a);
                upper_tri_mtx<ET> CM_inv_b= m_sphere_ET.circumsphere(supp_b);
    ...
    void set_simplex(const diameter_entry_t _simplex, const index_t _dim,
        ...
        m_sphere_ET.set(is_assembly,init_coboundary);
        m_sphere_PT.set(is_assembly,init_coboundary);
        m_2_sphere_PT.set(_simplex,init_coboundary);
        //construct simplex cache (CM of face-circumspheres)
#ifndef USE_RATIONALS
        if (dim>1 || (dim>0) && is_assembly && has_next(false)) {
            try {m_sphere_PT.construct_simplex_cache(vertices);}
    ...
    diameter_entry_t next(const bool is_assembly=false ,const bool init_coboundary=false) {
        ...
        if (simplex_degenerate) return diameter_entry_t(m_sphere_ET.construct(vertices,j--), cofacet_index, cofacet_coeff);
        try {
            if (dim>1 || (dim>0) && is_assembly) {
#ifdef USE_RATIONALS
                return diameter_entry_t(m_sphere_PT.construct(vertices, j--), cofacet_index, cofacet_coeff);
#else
                return diameter_entry_t(m_sphere_PT.construct_cofacet(vertices, j--), cofacet_index,  cofacet_coeff);
#endif
            }
            else return diameter_entry_t(m_2_sphere_PT.construct(vertices, j--,cofacet_index),cofacet_index, cofacet_coeff);
        }
        catch(CGAL::Uncertain_conversion_exception) {
            recomputation_count++;
            return diameter_entry_t(m_sphere_ET.construct(vertices, j+1), cofacet_index, cofacet_coeff);
        }
    }
};
